...
void randomPivot(int vetor[], int inicio, int fim)
{
    srand(time(NULL)); // inicializa o gerador de numeros aleatórios
    int random = inicio + rand()% (fim - inicio + 1);
    troca(vetor, inicio, random); // move o pivo para o inicio
}

int part(int vetor[], int inicio, int fim)
{
    randomPivot(vetor, inicio, fim);
